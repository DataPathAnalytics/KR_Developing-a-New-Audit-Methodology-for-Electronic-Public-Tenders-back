...
CREATE OR REPLACE FUNCTION update_krai_16()
  RETURNS VOID AS $$
DECLARE
  _tender                RECORD;
  _contract              RECORD;
...
BEGIN
  ...
  FROM tender t
  WHERE t.procurement_method_details IN ('simplicated', 'downgrade')
  ...
  LOOP
    ...
    FOR _contract IN SELECT * FROM contract WHERE tender_id = _tender.id AND status = 'active'
    LOOP

      FOR _award_id IN SELECT award_id FROM contract_award WHERE contract_id = _contract.id
      LOOP

        SELECT * FROM award WHERE id = _award_id INTO _award;

        SELECT count(*)
        FROM work_calendar
        WHERE is_working IS TRUE
          AND date > _award.date
      ...
      IF _indicator_value = PASSED THEN
        EXIT;
      END IF;
